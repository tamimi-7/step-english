...
const AR = require("./areas");
async function userLog(u){
  const exists = await db.call("SISMEMBER", "users", u);
  if(!(exists === 1 || exists === "1")) return null;
  ...
  const rounds = (results || []).map(x => { try{ return JSON.parse(x); }catch(e){ return null; } }).filter(Boolean)
    .map(r => ({ at: r.at, mode: r.mode, score: r.score, total: r.total, points: r.points || 0, base: r.base, bonus: r.bonus || 0, mult: r.mult || 1, stage: r.stage || null, story: r.story || null, partial: !!r.partial, challenge: !!r.challenge }));
  return {
