...
const THEMES = {
  ...
  5: { t: "جمعة المعركة", e: "⚔️", q: ["battle", "wordle", "new10"] }
};
...
function wordList(){
  ...
  const seen = new Map();
  (c.window.GEN_VOCAB || []).forEach(t => t.words.forEach(w => { const e = String(w[0]).toLowerCase(); if(/^[a-z]{5}$/.test(e) && !seen.has(e)) seen.set(e, { w: e, ar: w[1], ex: w[3] || "", exAr: w[4] || "", lvl: t.lvl }); }));
...
const WORDLE_PTS = tries => Math.max(1, 7 - tries);
async function wordleState(u, now){
  ...
  const family = Object.entries(db.flatToObj(allFlat)).map(([k, v]) => { try{ const x = JSON.parse(v); return { name: N[k] || k, me: k === u, tries: x.guesses.length, won: !!x.won, done: !!x.done, grid: x.done ? x.guesses.map(g => g.fb) : null }; }catch(e){ return null; } }).filter(Boolean)
  ...
  return { date, len: 5, max: 6, guesses: me.guesses, won: me.won, done: me.done, points: me.points || 0,
    ...
    hint: !me.done && me.guesses.length >= 4 ? ans.ar : null,
...
async function wordleGuess(u, guess, now){
  ...
  try{
    ...
    if(me.won){
      gained = WORDLE_PTS(me.guesses.length); me.points = gained;
...
module.exports = { questState, claimQuest, wordleState, wordleGuess, familyState, claimFamily, answerFor, feedback, wordList, THEMES, QUESTS };
