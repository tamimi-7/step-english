...
function build(){
  ...
  (W.STORIES || []).forEach(s => { (s.qs || []).forEach((_, i) => set.add(`st-${s.id}-${i}`)); (s.glossary || []).forEach(g => words.add(String(g.w).toLowerCase())); });
...
function isValid(id){
  ...
  if(/^(gw|gg|gd|vb|st)-/.test(id)) return X.set.has(id);
