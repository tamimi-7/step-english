...
function build(){
  ...
  (W.GEN_VOCAB || []).forEach(t => t.words.forEach((w, i) => { set.add(`gw-${t.id}-${i}`); words.add(String(w[0]).toLowerCase()); }));
  ...
  const stories = {};
  (W.STORIES || []).forEach(s => { (s.qs || []).forEach((_, i) => set.add(`st-${s.id}-${i}`)); (s.glossary || []).forEach((g, i) => { set.add(`sg-${s.id}-${i}`); words.add(String(g.w).toLowerCase()); }); stories[s.id] = { lvl: s.lvl, qs: (s.qs || []).length, gloss: (s.glossary || []).length }; });
  (W.CORE_WORDS || []).forEach(x => words.add(String(x[0]).toLowerCase()));
  return { set, words, stories };
}
function index(){
...
module.exports = { isValid, status, storyInfo };
