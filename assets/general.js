  ...
  function runQuiz(opts){
    ...
    const show = () => {
      ...
      render(`${opts.header || ""}<div class="quiz-top"><h2 style="margin:0;font-size:1.15rem">${opts.title}</h2><div class="btn-row"><span class="badge ok">${I("check")} ${S.score}</span><span class="badge info">${S.i + 1} / ${S.list.length}</span>${opts.timed ? `<span class="timer" id="gt">--</span>` : ""}</div></div>${bar(S.i / S.list.length * 100)}
        <div class="card q-card"><div class="btn-row" style="justify-content:space-between"><span class="badge">${q.label ? esc(q.label) : q.kind === "meaning" ? "ما معنى الكلمة؟" : q.kind === "reverse" ? "ما الكلمة الإنجليزية؟" : q.kind === "sentence" ? "أكمل الجملة" : q.kind === "grammar" ? "قاعدة: " + esc(q.sub || "") : q.kind === "dialog" ? "محادثة" : "سؤال"}</span><span class="q-tools">${q.lesson ? `<a class="mini-link rule-peek" href="#/lesson/${q.lesson}">${I("book")} شرح القاعدة</a>` : ""}${q.say ? spk(q.say) : ""}</span></div>
