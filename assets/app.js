...
function pointsExplain(j){
  ...
  if(j.review && j.review.points) rows.push(`<span class="px stage">${I("repeat")} مراجعة ${j.review.questions} ${j.review.questions === 1 ? "سؤال" : "سؤال"} = +${j.review.points}</span>`);
  if(j.review && !j.review.points && j.review.capped) rows.push(`<span class="px muted">${I("info")} خلّصت حد مكافأة المراجعة لهذا اليوم (٢٥ نقطة)، وتتجدد بكرة</span>`);
  ...
  if(j.repeated && !(j.review && j.review.points)) rows.push(`<span class="px muted">${I("repeat")} ${j.repeated} ${j.repeated === 1 ? "سؤال راجعته" : "سؤال راجعتها"}${j.review && j.review.capped ? "" : ` — المراجعة تعطي نقطة لكل ٤ إجابات${j.repeated < 4 ? "، باقي لك " + (4 - j.repeated) : ""}`}</span>`);
  ...
  if(!j.base && j.repeated && !(j.review && j.review.points)) rows.push(`<span class="px muted">${I("info")} أحسنت! المراجعة تعطيك نقطة لكل ٤ إجابات صحيحة، والنقاط الكاملة تلقاها في دروس ووحدات ما خلصتها</span>`);
