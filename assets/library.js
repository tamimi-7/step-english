  ...
  routes.read = id => {
    ...
    const draw = () => {
      ...
      <div class="player card" id="player">
        ...
        <div class="pl-main"><button type="button" class="btn btn-warm" id="focusBtn">${I("headphones")} وضع التركيز</button><div class="pl-speed-wrap"><span class="small muted">السرعة</span><div class="seg seg-sm pl-speed" id="speedSeg">${[0.8, 1, 1.2].map(v => `<button type="button" data-v="${v}" class="${(LIB.speed || 1) === v ? "on" : ""}">${v === 1 ? "عادي" : v < 1 ? "أبطأ" : "أسرع"}</button>`).join("")}</div></div></div>
        <details class="pl-more" ${window.__plMore ? "open" : ""} ontoggle="window.__plMore=this.open"><summary>${I("list")} خيارات القراءة: الترجمة، القاموس، المفردات</summary>
          <div class="pl-opt"><span class="small muted">الترجمة العربية</span><span class="seg seg-sm" id="arSeg"><button type="button" data-v="on" class="${arMode === "on" ? "on" : ""}">الترجمة</button><button type="button" data-v="tap" class="${arMode === "tap" ? "on" : ""}">عند الطلب</button><button type="button" data-v="off" class="${arMode === "off" ? "on" : ""}">بدون</button></span></div>
          <div class="btn-row pl-tools"><button type="button" class="btn btn-sm ${dictMode ? "on" : ""}" id="dictBtn">${I("search")} ${dictMode ? "القاموس مفعّل" : "اضغط كلمة لمعناها"}</button><button type="button" class="btn btn-sm" id="glossBtn">${I("type")} المفردات (${s.glossary.length})</button><button type="button" class="btn btn-sm ${listenMode ? "on" : ""}" id="modeBtn">${listenMode ? "أظهر النص" : "أخفِ النص"}</button></div>
